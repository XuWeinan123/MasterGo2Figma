...
| 样式条目 `06/0b` | — | 未破解 | 单字节 flag；2026-07-10 已破：`01`=decoration、`0c`=psName、`12`=styleName、`05` lineHeight `-1`=AUTO |
| 样式条目 `0e` | — | **证伪：不是 letterSpacing** | 插件测试.mg 的 `0e` 有 10–134 实值但基准 letterSpacing 全为 `{0,PERCENT}`；0710-2 恒 -1 但基准全为 `{0,PIXELS}`。letterSpacing 单位无逐条目/逐节点区分位（142 项 unit-only 残差视觉等价,不修） |
