  ...
  function hasManifestPath(zipEntries) {
    ...
    function mgDecodeNativeNodes(bytes) {
      ...
      for (let i = 0; i < marks.length; i++) {
        ...
        // Share exports: the run characters are authoritative (layer names
        // collapse newlines, e.g. "SPEED\nLIMIT" → "SPEED LIMIT"). Full
        // exports keep the old conservative rule for rich-text edge cases.
        if (textDetails.characters && (mgShareModeActive || !name || name.indexOf("_") >= 0)) nodes[mk.recId].characters = textDetails.characters;
