  ...
  function hasManifestPath(zipEntries) {
    ...
    function mgParsePaintRecord(bytes, start, end) {
      ...
      if (kind >= 1 && kind <= 4 && gradient && gradient.stops && gradient.stops.length >= 2) {
        ...
        // extended sample would need a new discriminator. Bare-03 records
        // store the ratio directly (mgRadialAxisRatio).
    ...
    const MG_ALIGN_ITEMS = { 1: "MAX", 2: "CENTER", 3: "MAX", 4: "SPACE_BETWEEN" };
      ...
      for (const sid in styleDefs) {
        ...
        if (def.category === "FILL" || def.category === "STROKE") {
          ...
          if (fills && fills.length) {
            emittedStyles.push({ id: sid, styleType: "PAINT", name: def.name, paints: fills.map(mgCloneJsonValue) });
          }
