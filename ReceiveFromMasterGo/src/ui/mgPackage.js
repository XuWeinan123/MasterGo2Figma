(function(global) {
  ...
  function hasManifestPath(zipEntries) {
    ...
    function mgReadTransformXY(bytes, start, end) {
      ...
      for (let off = start; off < end - 5; off++) {
        ...
        if (mode === 0x01) {
          ...
          // Matrix fields 03=m00 04=m11 05=m01 06=m10 are optional: only
          // non-default entries are stored (defaults m00=m11=1, m01=m10=0).
          // E.g. a 180°-rotated group stores just `03 <-1.0>`.
          ...
          if (sawMatrix &&
            ...
            let rotation = Math.atan2(m01, m00) * 180 / Math.PI;
            if (rotation > 180 - 1e-6) rotation -= 360; // Figma reports 180° as -180
    ...
    const MG_IMAGE_SCALE_MODE = { 0: "FILL", 1: "FIT", 2: "CROP", 3: "TILE" };
    ...
    function mgParsePaintRecord(bytes, start, end) {
      ...
      if (kind >= 1 && kind <= 4 && gradient && gradient.stops && gradient.stops.length >= 2 && gradient.p0 && gradient.p1) {
        const transform = kind === 1
          ? mgLinearGradientTransform(gradient.p0, gradient.p1)
          : mgRadialGradientTransform(gradient.p0, gradient.p1, gradient.ratio);
        return {
    ...
    // terminator.
    const MG_OWNER_TYPE_RE = /\x1b[0-9]+:[0-9A-Za-z]+\x00\x1c/;
    ...
    function mgWalkScalarFields(bytes, fb, scalEnd, afterName) {
      ...
      while (p < end) {
        const t = bytes[p];
        if (t === 0x05 || t === 0x08 || t === 0x09 || t === 0x0b || t === 0x0c || t === 0x0d ||
    ...
    const MG_CONSTRAINT = ["START", "CENTER", "END", "STRETCH", "SCALE"];
    ...
    function mgParseContainerMeta(bytes, off) {
      ...
      if (bytes[p] === 0x06 && meta.subtype === "FRAME") { meta.subtype = "INSTANCE"; return meta; }
      ...
      if (bytes[p] === 0x09) {
        ...
        // MasterGo omits the field entirely when the runtime default (10)
        // applies; an explicit zero is written as `09 00`.
        meta.itemSpacing = 10;
      }
      if (bytes[p] === 0x0a) {
        p++;
        ...
        // Same default rule: an empty padding object means "all 10".
        meta.paddings = padCount === 0
          ? { top: 10, right: 10, bottom: 10, left: 10 }
          : { top: pads[1] || 0, right: pads[2] || 0, bottom: pads[3] || 0, left: pads[4] || 0 };
      }
    ...
    function mgDecodeGeometryBlob(bytes, pos) {
      ...
      function regionRecord() {
        ...
        for (;;) {
          ...
          const t = bytes[p++];
          if (t === 0x00) return rec;
          if (t === 0x01) { const loop = intArray(); if (!loop) return null; rec.loops.push(loop); continue; }
          if (t === 0x02) { rec.index = varint(); continue; }
    ...
    function mgDecodeNativeNodes(bytes) {
      ...
      for (let i = 0; i < marks.length; i++) {
        ...
        // TEXT object leading field `03` = textAutoResize
        // (0=WIDTH_AND_HEIGHT 1=HEIGHT; omitted=NONE/fixed size).
        let textAutoResize = null;
        if (typeByte === 8) {
          textAutoResize = bytes[fb + jt + 2] === 0x03
            ? (bytes[fb + jt + 3] === 1 ? "HEIGHT" : "WIDTH_AND_HEIGHT")
            : "NONE";
        }
        ...
        nodes[mk.recId] = {
          ...
          isMask: scalar.t09 === 1,
          constraintH: scalar.t0b || 0, constraintV: scalar.t0c || 0,
      ...
      return { nodes: nodes, paints: paints, geoms: geoms };
    }
    ...
    function mgApplyCommonVisualDefaults(props) {
      ...
      if (Array.isArray(props.blend.effects)) {
        props.blend.effects.forEach(e => {
          if (e && (e.blendMode === "NORMAL" || !e.blendMode)) e.blendMode = "PASS_THROUGH";
        });
      }
      if (props.geometry) {
    ...
    function mgNativeProps(n, nodes, paints, geoms) {
      ...
      const strokeWeight = n.strokeWeightExplicit ? n.strokeWeight : (n.strokeWeight || 1);
      const meta = n.containerMeta;
      const cornerRadius = (meta && meta.corners) ? 0 : (n.cornerRadius || (n.cornerRef ? 10 : 0));
      const trailer = n.trailer || {};
      const sideWeights = trailer.sideWeights || [strokeWeight, strokeWeight, strokeWeight, strokeWeight];
      const props = {
        ...
        blend: { opacity: 1, isMask: !!n.isMask, blendMode: "NORMAL", effects: [] },
        corner: {
        ...
        geometry: {
          ...
          dashPattern: n.dashPattern ? n.dashPattern.slice() : [],
          strokeCap: MG_STROKE_CAP[trailer.strokeCap] || "NONE"
        },
        layout: {
      ...
      if (t === "TEXT") {
        ...
        props.textAlignHorizontal = "LEFT"; props.textAlignVertical = "TOP";
      ...
      if (meta) {
        ...
        if (types.sourceType === "BOOLEAN_OPERATION") {
          ...
          props.clipsContent = meta.clipsContent === undefined ? false : meta.clipsContent;
        } else if (frameFamily) {
        ...
        if (meta.itemSpacing !== undefined && types.type !== "SECTION") props.layout.itemSpacing = meta.itemSpacing;
        if (meta.paddings && types.type !== "SECTION") {
          props.layout.paddingTop = meta.paddings.top;
          props.layout.paddingRight = meta.paddings.right;
          props.layout.paddingBottom = meta.paddings.bottom;
          props.layout.paddingLeft = meta.paddings.left;
        }
        ...
        // omitted = AUTO. Instances inherit from their component instead.
        if (n.trailer && types.sourceType !== "INSTANCE") {
    ...
    function convertMgPackageToV2Entries(zipEntries, fileName) {
      ...
      const { nodes, paints, geoms } = mgDecodeNativeNodes(documentBytes);
      ...
      for (const id in reachable) {
        const n = nodes[id];
        const props = mgNativeProps(n, nodes, paints, geoms);
