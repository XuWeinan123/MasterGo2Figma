...
//
// Usage: node dump_records.js <repoRoot> <file.mg> <baseline.zip> <outDir>
...
const [repoRoot, mgArg, zipArg, outArg] = process.argv.slice(2);
if (!repoRoot || !mgArg || !zipArg || !outArg) {
  console.error("Usage: node dump_records.js <repoRoot> <file.mg> <baseline.zip> <outDir>");
  process.exit(2);
}
const mgPath = path.resolve(repoRoot, mgArg);
const zipPath = path.resolve(repoRoot, zipArg);
...
const mgPackagePath = path.join(repoRoot, "ReceiveFromMasterGo", "src", "ui", "mgPackage.js");
